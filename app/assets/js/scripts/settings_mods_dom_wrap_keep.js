...
// === Mods Dropdown (No EJS) — keeps original nodes to preserve cached references ===
(function(){
  ...
  function init(){
    ...
    // A mount container that contains the three sections; fall back to current parent
    const mount =
    ...
    function createSection(key, title, contentEl){
      ...
      // === IMPORTANT: keep the original node instance! ===
      // Move the EXISTING contentEl into our inner container (do not replace it).
      ...
      return { section, badge, holder: contentEl };
    }
    ...
    // Insert sections into the original parent of the first container to keep layout stable
    const parent = sections.length ? sections[0].holder.parentElement.parentElement : mount;
